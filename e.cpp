...
void NaviFollowState::exec(Navi* navi)
{
	...
	if (!isNear && (state == NSID_Throw || state == NSID_ThrowWait)) {
		// this float is literally 0x1 off in the sdata2
...
void NaviThrowWaitState::exec(Navi* navi)
{
	...
	NaviParms* parms     = static_cast<NaviParms*>(navi->mParms);
	navi->_2B4           = _1C / 3.0f * (parms->mNaviParms.mThrowDistanceMax.mValue - parms->mNaviParms.mThrowDistanceMin.mValue)
	           + parms->mNaviParms.mThrowDistanceMin.mValue;
	parms      = static_cast<NaviParms*>(navi->mParms);
	navi->_2B8 = _1C / 3.0f * (parms->mNaviParms.mThrowHeightMax.mValue - parms->mNaviParms.mThrowHeightMin.mValue)
	           + parms->mNaviParms.mThrowHeightMin.mValue;
	...
	if (input & Controller::PRESS_DPAD_RIGHT || input & Controller::PRESS_Y) {
		...
		int pikisNext[6];
		// for (int i = 1; i != 7; i++) {
		pikisNext[0] = ((type + 1) % 7);
		pikisNext[1] = ((type + 2) % 7);
		pikisNext[2] = ((type + 3) % 7);
		pikisNext[3] = ((type + 4) % 7);
		pikisNext[4] = ((type + 5) % 7);
		pikisNext[5] = ((type + 6) % 7);
		//}
		...
		int pikisNext[6];
		// for (int i = 1; i != 7; i++) {
		pikisNext[0] = ((type + 6) % 7);
		pikisNext[1] = ((type + 5) % 7);
		pikisNext[2] = ((type + 4) % 7);
		pikisNext[3] = ((type + 3) % 7);
		pikisNext[4] = ((type + 2) % 7);
		pikisNext[5] = ((type + 1) % 7);
		//}
	...
	if (input & Controller::PRESS_DPAD_UP || input & Controller::PRESS_DPAD_DOWN
	...
	} else if (!(navi->mController1->getButton() & Controller::PRESS_A)) {
		sortPikis(navi);
		navi->mHoldPikiTimer = _1C / 3.0f * (parms->mNaviParms.mThrowDistanceMax.mValue - parms->mNaviParms.mThrowDistanceMin.mValue)
		                     + parms->mNaviParms.mThrowDistanceMin.mValue;
	...
	if (thePlate->mActiveGroupSize > 0) {
		...
		Vector3f naviPos = dog - navi->getPosition();
		thePlate->setPos(naviPos, navi->mFaceDir + PI, navi->mSimVelocity, 1.0f);
		sortPikis(navi);
	}
...
void NaviThrowWaitState::sortPikis(Navi* navi)
{
	...
	navi->mCPlateMgr->setPos(naviPos, navi->mFaceDir + PI, navi->mSimVelocity, 1.0f);
